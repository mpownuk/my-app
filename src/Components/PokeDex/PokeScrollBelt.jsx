...
export const PokeContainer = ({pokeList, pictures, func}) => {
    ...
    return (
        <div className="poke--scrollbelt">
            <Prev className='hidden--button' value={'<'} onClick={()=>{moveBar(movableEl.current, 250, -10)}}/>
            ...
            <Next className='hidden--button' value={'>'} onClick={()=>moveBar(movableEl.current, 250, 10)}/>
        </div>
    )
}